...
test("reclaim-done appends the est→real calibration clause when both figures are present AND costUsd is known-real, on either branch", () => {
  assert.equal(
    render("reclaim-done", { worker: "w1", next: "DRIVING", estCostUsd: 6.21, costUsd: 5.8, costEstimated: false }),
    "Lane w1 opened a PR — now in review · est $6.21 → real $5.80",
  );
  assert.equal(
    render("reclaim-done", { worker: "w1", next: "REQUEUE", estCostUsd: 6.21, costUsd: 5.8, costEstimated: false }),
...
test("reclaim-done renders no calibration clause when estCostUsd is absent — a lane never probed while running", () => {
  assert.equal(
    render("reclaim-done", { worker: "w1", next: "DRIVING", costUsd: 5.8, costEstimated: false }),
    "Lane w1 opened a PR — now in review",
  );
});

test("reclaim-done renders no calibration clause when costUsd is absent (a pre-#890 payload) — never half a figure", () => {
  assert.equal(
    render("reclaim-done", { worker: "w1", next: "DRIVING", estCostUsd: 6.21, costEstimated: false }),
    "Lane w1 opened a PR — now in review",
  );
});

test("reclaim-done renders no calibration clause when costUsd is itself an estimate — never labels an estimate as real", () => {
  assert.equal(
    render("reclaim-done", { worker: "w1", next: "DRIVING", estCostUsd: 6.21, costUsd: 5.8, costEstimated: true }),
    "Lane w1 opened a PR — now in review",
  );
});

test("reclaim-done renders no calibration clause when costUsd's provenance is unknown (costEstimated absent) — never a guessed 'real'", () => {
  assert.equal(
