...
/** §7: plain word first, internal term never. Reserved — see the render comment below. */
const PLANNING_NODES = [
  { y: 96, label: "Goal & align", hint: "Decides what's worth doing this round and files it as issues" },
  { y: 158, label: "Arch review", hint: "Checks the round's plans fit the architecture before work starts" },
  {
    y: 220,
    label: "Verify",
...
export function HeroStage({ state, fixCap, dimmed = false, reducedMotion = false, ref }: HeroStageProps) {
  ...
  return (
    ...
    >
      {/* ── Phase captions ── */}
      <text className="hero-phase" x={176} y={26} textAnchor="middle">
        PLAN
      </text>
      <text className="hero-phase" x={620} y={26} textAnchor="middle">
        IMPLEMENT
      </text>
      <text className="hero-phase" x={1030} y={26} textAnchor="middle">
        OUTCOME
      </text>
      ...
      <g className="hero-backlog">
        ...
        {state.pool.map((issue, i) => (
          <g className="hero-pool-chip" key={issue} data-issue={issue}>
            <rect x={BACKLOG.x + 8} y={BACKLOG.y + 10 + i * BACKLOG.chip} width={BACKLOG.w - 16} height={16} rx={8} />
      ...
      {/*
       * ── Zone 2: planning ──
       * Reserved, permanently. §6 wants these nodes lit from the round phase cursor, but no
       * persisted event carries it yet (`round-phase` is a pending engine follow-up, §11), and
       * a node that animates without a data source is exactly the fake progress §6 forbids.
       */}
      <g className="hero-reserved" data-node="planning" data-reserved="true">
        {PLANNING_NODES.map((node) => (
          <g key={node.label}>
            <title>{node.hint}</title>
            <circle cx={PLANNING.x} cy={node.y} r={17} />
            <text className="hero-node-label" x={PLANNING.x + 28} y={node.y + 4}>
              {node.label}
            </text>
          </g>
        ))}
        <text className="hero-reserved-note" x={PLANNING.noteX} y={PLANNING.note}>
          planning roles — coming with rounds
        </text>
      </g>
      ...
      <g className="hero-escalation" data-node="needs-human" data-count={escalated}>
        <path className="hero-branch" d={`M ${ESCALATION.x} ${GATES.y} L ${ESCALATION.x} ${ESCALATION.y - 18}`} />
        <circle cx={ESCALATION.x} cy={ESCALATION.y} r={13} />
      ...
      <g className="hero-trunk" data-rings={state.rings}>
        {ringRadii(state.rings).map((r, i, all) => (
          <circle className="hero-ring" key={r} cx={TRUNK.x} cy={TRUNK.y} r={r} data-current={i === all.length - 1 ? "true" : "false"} />
        ))}
        <text className="hero-ring-count" x={TRUNK.x} y={TRUNK.y + 106} textAnchor="middle">
          {state.rings}
        </text>
      ...
      <g className="hero-reserved" data-node="reflection" data-reserved="true">
        {REFLECTION_NODES.map((node) => (
          <g key={node.label}>
            <circle cx={REFLECTION.x} cy={node.y} r={13} />
            <text className="hero-node-label" x={REFLECTION.x} y={node.y + 30} textAnchor="middle">
              {node.label}
            </text>
          </g>
        ))}
