...
/** #728: caps the cluster's rightward spread so it stays clear of the trunk rings (leftmost
 *  extent `TRUNK.x - TRUNK.max * TRUNK.step` = 922) and the OUTCOME tally text — a wide
 *  escalation list wraps into a new row (upward, away from the ESCALATION node) instead of
 *  running into either. */
const NEEDS_HUMAN_COLS = 3;
const NEEDS_HUMAN_ROW_STEP = 22;
...
export function dropletPoint(state: HeroState, d: Droplet, at: DropletAt = d.at): { x: number; y: number } {
  switch (at) {
    case "backlog": {
      // #728: shares ONE vertical counter with the pool chips drawn in the same narrow
      // column (stage.tsx's `state.pool.map`) — a backlog droplet's row starts only after
      // every pool chip's own row, so the two lists can never land on the same slot. A
      // handed-off droplet carries a second line (the "saved for a successor" badge), so it
      // claims two rows' worth of room rather than crowding whatever comes after it.
    ...
    case "needs-human": {
      ...
      return { x: ESCALATION.x + col * 26, y: ESCALATION.y - 30 - row * NEEDS_HUMAN_ROW_STEP };
    }
    case "trunk":
