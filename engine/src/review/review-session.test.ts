...
test("end-to-end: the review session's tool profile is Read/Grep/Glob only, no Bash, no forge proxy — even when a RoleRunnerDeps.defaultProxy is configured for OTHER (non-review) role sessions", async () => {
  ...
  try {
    ...
    assert.equal(
      at("--setting-sources"),
      "",
      "ZERO file settings sources load (not user/project/local) — only the inline guard --settings; the materialized tree's own settings AND the producer-influenceable user ~/.claude/settings.json are both excluded",
    );
  } finally {
    cleanup();
    rmSync(dir, { recursive: true, force: true });
