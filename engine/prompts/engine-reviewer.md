...
human-witnessed probe may reach `confirmed` only against the probe RECORD in the issue **body**,
as read from the snapshotted issue body above (actor, steps, timestamp, artifact) — a comment is
never visible to this session, whatever it says. A criterion whose record exists only as a
comment stays `cannot-confirm`, and that gap is the operator's, not the producer's
(docs/REVIEW-DOCTRINE.md's tier-C doctrine) — never against PR-body narration describing what was
