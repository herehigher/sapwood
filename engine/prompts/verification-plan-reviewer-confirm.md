...
them, remain `<!-- sapwood:ac -->` and `<!-- sapwood:verification -->`. New prose with no existing
content to match defaults to the configured working language `{{lang.issuesAndPrs}}`.
...
unless it fits an exception `engine/prompts/doctrine-core.md`'s test-realism section names (a second,
