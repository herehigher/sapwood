...
| `engine-review-containment-gap` | Recorded limit, not an incident: this review ran in a sandbox that blocks writes but still lets the reviewed code be executed (#443, `payload.gap`). Emitted at every local-Codex review start so the limit is on the record rather than assumed away — not an attention item, and never a reason on its own to stop a lane |
