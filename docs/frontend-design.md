...
| `reclaim-done` | Branches on `payload.next`: PR produced → "Lane {worker} opened a PR — now in review"; ended without a PR → "Lane {worker} ended without a PR — {reason, or "reason not recorded" — #881 payload gap, see below} · asks: review the lane's outcome and decide whether to retry". #890: either branch appends " · est $X → real $Y" when the payload carries both `estCostUsd` (the lane's last live estimate) and `costUsd` (the settled real figure) — omitted whenever either is absent, never a fabricated half-figure. |
