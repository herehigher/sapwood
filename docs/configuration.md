...
| `poolFactor` | `1.5` | #212: the aligning phase selects up to `ceil(lanes.roundDispatchCap × poolFactor)` issues from Ready (milestone-scoped when `milestone` above is set), ordered by `prio:*` label ascending then issue number ascending, and labels them `labels.roundPool` — this round's dispatch-eligible pool. The executing phase dispatches pool members only (an approved Ready issue outside the pool is never dispatched that round); the standby probe still counts an un-pooled Ready issue as work. `>1` so the pool absorbs gate⓪/review attrition between selection and dispatch. Runs every round regardless of `roles.po.enabled` — with the PO off, selection is still engine-computed (`top cap × factor of Ready by prio`, no session): the selection bound never depends on an optional role. The pool label persists through dispatch (a dead-lane requeue inside the same round stays pooled) and is cleared from any still-undispatched member at round close, returning it to plain Ready for the next round's selection. |
