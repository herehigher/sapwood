...
| Key | Default | Meaning |
|---|---|---|
| `forgeCallTimeoutMs` | `60000` | Hard per-call ceiling on a single `gh` CLI invocation (`gh.ts` — the one place the engine shells out to `gh`; every `GithubForge` call routes through it). A dead socket/hung upstream fails toward retry instead of wedging the tick loop forever — every GithubForge caller either lives inside `tick()` (contained by the tick-error path) or has its own local try/catch; none can reach a park/fatal-exit path. 60s (not `proxy.timeoutMs`'s 30s) gives headroom for `gh api --paginate` reads over potentially large pages (`getCommitsSince`, `getPRReviewData`, `getIssueComments`). |
