...
| Key | Default | Meaning |
|---|---|---|
| `mode` | `engine-agent` (#501; was `different-model-codex` before 2026-08-01) | The reviewer kind: `engine-agent` (an engine-composed, static review session using a different Claude model, run locally on the same Claude CLI sapwood itself needs — no extra install), `different-model-codex` (a fresh non-author Codex review, requiring the hosted `@codex review` GitHub App), `same-model-trusted` (allowlisted reviewers only), or `human` (any non-author approval). **Cost note:** the `engine-agent` default makes gate② a paid local Claude session — see `agent.costCapUsd` below for its per-review ceiling. |
