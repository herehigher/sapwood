...
| `poolFactor` | `1.5` | #212: the engine computes this round's pool CANDIDATE set from Ready (milestone-scoped when `milestone` above is set) — up to `ceil(lanes.roundDispatchCap × poolFactor)` issues, ordered by `prio:*` label ascending then issue number ascending. With `roles.po.enabled: true` (the default), the PO's dedicated selection session (`roles.po.poolPromptFile`) then chooses which of those candidates actually belong in this round's pool — it may take all of them, a subset, or (rarely) none; with `roles.po.enabled: false`, the full candidate set IS the pool (the documented AC7 fallback, no session). Either way the engine applies `labels.roundPool` to exactly the resulting selection, from validated output only (never a label name the session could name directly). The executing phase dispatches pool members only (an approved Ready issue outside the pool is never dispatched that round); the standby probe still counts an un-pooled Ready issue as work. `>1` so the candidate set absorbs gate⓪/review attrition between selection and dispatch. The pool label persists through dispatch (a dead-lane requeue inside the same round stays pooled) and is cleared from every still-undispatched member — anywhere in the open backlog, not just still-Ready issues — at round close, returning it to plain Ready for the next round's selection. |
...
| `po.backlogDigestMaxChars` | `20000` | Hard cap, in characters, on the milestone-scoped open-issue digest substituted into PO align prompts as `{{backlog.digest}}`. The engine assembles issue numbers, titles, and configured human-hold label annotations at invocation time; zero issues and read failure render distinct explicit notes. Cuts are deterministic and marked. Minimum `200`. Custom PO prompt files may omit the variable. #212: also reused, unmodified, as the cap on the round-pool selection session's candidate digest (`poolPromptFile` below) — naturally far smaller (bounded by the pool cap), so this is a safety valve there too. |
| `po.poolPromptFile` | unset | #212: override the round-pool **selection** session's prompt — a separate template from `po.promptFile` (align/triage). Same `#74` pattern: a relative path resolves against the config file's own directory. Unset uses the engine's shipped `prompts/po-pool.md`. Every round (with `roles.po.enabled: true`), this session receives the engine-computed candidate digest — the top `ceil(lanes.roundDispatchCap × round.poolFactor)` Ready issues, prio-ordered — and returns which of those issue NUMBERS belong in this round's pool; the engine applies `labels.roundPool` to exactly that (validated) selection. The output schema carries issue numbers only — no label name ever appears in it, so the round-pool label-removal containment invariant (see the `labels` table above) cannot be affected by this session's output even in principle. |
| `po.enabled` | `true` | #127: switch the `aligning` phase's PO **sessions** (goal-alignment + triage, and — with this off — the round-pool selection session too) off for this deployment. `false` → both the align/triage session and the pool-selection session are skipped; round-pool selection instead runs its documented deterministic fallback (`top cap × factor of Ready by prio`, no session) every round regardless — the selection *bound* must never depend on an optional role (#212 AC7). The align/triage side still OMITS its own stub the same way as before: `round-defaults.ts`'s `createDefaultPeripherals` skips just that portion, never the whole `aligning` phase — see `round.poolFactor` above for the pool-selection mechanics. **Warning:** with the PO off, plan-less issues are never triaged into the gate⓪ pipeline (no plan drafting, no decomposition) — they must arrive with a verification plan already in the body, or a human/external process must draft one, before gate⓪ can ever approve them. |
