...
| `retro.digestMaxChars` | `60000` | Hard cap, in characters, on the engine-built read digest substituted into retro's prompt as `{{round.digest}}`. Most of it is round-scoped — PR diffs + review signals for every PR the round touched, comments/labels for every escalated issue, and the round's commit history — but the digest's `Your outstanding PRs` section (#964) is NOT: it walks retro's WHOLE PR history (every PR it has ever opened, not merged/closed yet) and shares this SAME cap under its own reserved fraction, so a red PR from three rounds ago stays visible even on a quiet round. For an actionable (red-CI) row, the failure excerpt itself is fetched via `IForge.getFailedCheckSummary` (GitHub's checks API), which carries its own internal hard cap BEFORE this digest-level one is applied on top. Oversize digests are truncated **deterministically** (same prefix every time for the same content+cap) and the cut is marked in the digest text itself, never silently dropped. No new config key for the outstanding-PRs section — it shares this one cap, at the cost of a smaller reserved share for the sections that existed before #964. |
