...
| Key | Default | Meaning |
|---|---|---|
| `llmPatterns` | see `sapwood.config.yaml` | Regex patterns (case-insensitive, compiled/validated at load, non-empty) matched against a FAILED lane's structured error output to classify it as an LLM-provider environment failure — `rate_limit_error`, `usage limit reached`, `credit balance is too low`, `insufficient_quota`, `overloaded_error`, `429 too many requests`, `hit your \S+ limit` (the Claude CLI's own session/weekly/5-hour plan-quota-exhaustion message family, #394), etc. Also checked BEFORE any text pattern: a rejected structured `rate_limit_event` in the session transcript classifies as `llm` unconditionally (#394) — text patterns are the fallback for providers/CLIs that don't emit that telemetry. |
