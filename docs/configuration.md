...
| `split` | `sapwood:split` | Human firing signal for one PO-decompose generation. The engine reads but never applies or removes it. Applying it to an `origin:agent` child is the explicit fresh signature that permits one further generation. |
