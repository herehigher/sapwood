...
| `deployKeyPath` | unset | (#606 gate② round 1, [#351 final ruling](security.md#worker-credential-tiers-351-606)) Path to the per-repo SSH **write deploy key** `sapwood init` provisions — set TOGETHER with `deployKeyId` below (they are the LOCAL anchor pair the owner ruling reconciles against; a title is never authoritative for "mine"). Both set + reconciled green activates **L1**: every worker leg — dispatch, resume, AND fix — runs with `GIT_SSH_COMMAND` pinned to this key (composed onto the credential-free base for a fix leg, never replacing its severing) and no forge API credential reachable in its env at all (`Bash(gh *)` drops out of the leg's tool grant too). Same relative-path resolution as `promptFile`. Unset (the default) is **L0** — today's full credentialed env, unchanged. A reconcile failure (missing local file, a rotated/foreign remote id, an SSH auth failure) never blocks dispatch: the engine logs a guidance-carrying WARN, clears the stale anchor, and every leg proceeds at L0 until a human either fixes the underlying issue or re-runs `sapwood init` interactively to register an additional per-machine key. See [Security & trust model](security.md#worker-credential-tiers-351-606) for the full tier table, the reconcile state machine, and residuals. |
| `deployKeyId` | unset | (#606 gate② round 1) The GitHub-assigned numeric id of the deploy key at `deployKeyPath` above — written by `sapwood init` alongside it, and always set/cleared TOGETHER with it. This is the other half of the local `(path, id)` anchor `sapwood init`'s reconcile pass keys on; the bare key TITLE on the repo is never treated as proof of ownership (a `sapwood-worker`-titled key may validly belong to a different machine). Never hand-edit this without also updating `deployKeyPath` — an anchor with only one half set behaves as "nothing configured" (fresh provisioning) on the next `sapwood init` run. |
