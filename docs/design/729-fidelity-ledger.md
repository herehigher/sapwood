...
| 9 | header | Hero "?" legend discoverability | PM routing comment: "hero glossary hidden behind an unlabeled '?'" | `Legend.tsx`'s `<button aria-label="Legend">?</button>` (a Radix `Popover.Trigger`, #892 — was a native `<summary>`) — labeled for assistive tech; the bare "?" glyph is `frontend-design.md` §7's own deliberate ruling ("that is the whole onboarding surface — no tour, no modal sequence") | adjudicated-keep — intentional per existing design doc ruling, not an oversight; visual minimalism is the point |
