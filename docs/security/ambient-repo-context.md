...
| Source content | every present source hashed + stored inline, content-addressed, even a git-tracked file; absent (confirmed nonexistent) vs. unreadable (a real blind spot) distinguished | `context-manifest.ts::assembleContextManifest` |
| auto-memory | `MEMORY.md` under the session's self-reported memory path | `peripheral.ts::assembleManifest` |
| Model / CLI / tools | model, CLI version, a tool-name-set hash, a rendered-prompt hash; `modelSource` flags a fallback-model switch | `worker.ts::parseSessionInit` |
| MCP servers | name + connection status per server, from the init report's `mcp_servers` field, never the tool-name list (`mcp__`-prefixed schemas arrive deferred, after init) | `context-manifest.ts::assembleContextManifest` |
...
| Host posture | the session's own init-reported effective `permissionMode`; a count of denied-command `<sandbox_violations>` blocks, diagnostic only, never gating the session's own outcome | `worker.ts::countSandboxViolations` |
...
- **Not chased:** `@import` directives inside any probed file, ancestor-directory `CLAUDE.md`
  ...
  (`context-manifest.ts`: `assembleContextManifest (Codex F2b): probedPaths/knownUnprobed pass
  through verbatim`).
...
- `dirty` is derived, never measured: `"structural-no-write-tools"` vs. `"unknown-write-capable-session"`
  (conservatively `true`) turns on whether the effective grant names a write-capable tool, e.g.
  `retro`'s `Write`/`Bash(git ...)` (`peripheral.ts::hasWriteCapableGrant`) — an ambient MCP
  server's own write-capable tools are still invisible to this name-based check. `"worktree-missing"`
  is separate: the worktree never appeared on disk at capture time
  (`context-manifest.ts::assembleContextManifest`).
- `git status`/`diff` is never run against a session-controlled worktree: an empirically confirmed
  worker-set `filter.<name>.clean` turns that into code execution (the #65 RCE class,
  `worker.ts::retainOrDeleteWorktree`'s doc; `worker.test.ts`: "drain (SIGTERM) ... NO git
  subprocess is spawned"). HEAD/dirty are pure-filesystem derivations instead.
...
- The engine always REQUESTS one fixed `host.permissionMode`, but the CLI can silently fall back to
  ...
  engine-visible signal unless this field is read. A divergence emits one `permission-mode-mismatch`
  event — informational only, fail-safe-allow, never gating the lane's outcome
  (`worker.ts::permissionModeMismatched`). Whether the Bash sandbox actually engaged has no
  equivalent positive signal — left to DR #1009.
