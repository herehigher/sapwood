...
**Supervisor prerequisite.** Operators running unattended under a supervisor MUST configure the
supervisor's own crash-loop circuit-breaker (e.g. systemd's `StartLimitBurst=5`/
`StartLimitIntervalSec=600`) — sapwood assumes it and alerts nowhere else. The engine's own
rapid-restart detector (`engine.rapidRestart`, default 5 starts/10min) and the single-instance
data-dir lock are defense-in-depth behind that assumption, not a substitute; a crash loop's blast
radius is still bounded by `dailyBudgetUsd` and the merge gates.
