...
without becoming a gate (`worker.test.ts`: "egress tripwire failed (non-fatal)"). This is a
...
(`worker.ts::classifyEgressTarget`; `worker.test.ts`: "a loopback hit carries target:'loopback' in
the payload; a public hit's payload is unchanged"). See [Design
derivations](../design/security-egress-derivations-2026-08.md) for the motivating observation.
...
see `worker.test.ts`: "classifyEgressTarget (#387): loopback URL matrix" for the exact host forms
covered and its non-matches.
...
a benign hit, while the opposite error would downgrade a real one. Classification reads the full
observed text, not the 200-character evidence snippet, so a public URL truncated out of the
recorded evidence cannot be tagged loopback.
...
process itself performs, and not every inherited schema's semantics. Gate②'s review-session mode
also refuses a caller-supplied `allowedTools` outright (thrown, not silently accepted).
...
- **No model-invoked egress beyond provider transport.** The session is pinned to `--sandbox
  ...
  `--ignore-user-config`, `--ignore-rules`, and the hardened env below. The prompt reaches the
  CLI on stdin from a file; the module spawns an argv vector, never a shell. The wall-clock
  session timeout terminates the whole process group.
...
- **Recorded blind spot 3 — the inherited environment.** The session's environment is the
  engine's own, minus a denylist. `codexSessionEnv` strips well-known credential families — forge
  tokens, `SSH_AUTH_SOCK`/`SSH_AGENT_PID`, AWS/GCP/Azure, `KUBECONFIG`, npm/pip/twine/cargo
  registry tokens, Docker — plus a sweep of any variable ending `_TOKEN`, `_SECRET`, `_API_KEY`,
  `_PASSWORD`, or `_CREDENTIALS` (`isStrippedEnvKey`).
...
deliberately open channel that must not be sealed as a side effect. See [Design
derivations](../design/security-egress-derivations-2026-08.md) for the rejected pinning approach.

Instead `cli.ts::checkWebAccessSettingsDenial` — called from the same best-effort startup pass as
`assertStopMilestoneExists` — reads ONLY the operator's user-level settings
...
A hit records an attempt, never proof of execution — the CLI's own permission layer may still
deny it at the paired `tool_result`, which this scanner does not read. `RoleRunner.run()` calls it
on every session's own completed jsonl; `round-artifact.ts`'s existing assembler needs no changes
to surface either kind.
