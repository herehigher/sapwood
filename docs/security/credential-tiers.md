...
`sapwood init` provisions the L1 key when the operator has repo-admin, degrading to a guidance WARN on any failure — never blocking `init` itself. Every condition this enforces, and the test pinning it, is in the table below.
...
| `sapwood run` startup reports the effective tier via both the log and a `deploy-key-tier-detected` event (`record()`) before any dispatch, and fails closed for `L1` with no reconciled anchor: no local anchor, an unreadable key, a stale/read-only/unlisted remote id, or a failed preflight. | `deploy-key-startup-check.ts::detectDeployKeyStartupTier`, called from `cli.ts` right after `WorkerSupervisor` construction | `deploy-key-startup-check.test.ts`: "arm 2: credentialTier L1, no local anchor"; "arm 3: credentialTier L1, anchor found but key file unreadable"; "arm 4a: credentialTier L1, anchor readable but remote id no longer listed"; "arm 4b: credentialTier L1, anchor's remote id is registered read-only"; "arm 5: credentialTier L1, anchor found, reconciled remotely, SSH preflight fails" |
| A still-running lane from before this restart whose own persisted `credential_tier` doesn't match `L1` (a marker with none recorded, an unparseable marker, or an unlistable state directory all count) blocks startup, naming each lane/session/pid/tier and exactly two remedies (wait it out, or `kill <pid>`) — never `sapwood estop`. | `detectDeployKeyStartupTier`'s running-marker scan, `worker.ts::listRunningCredentialTiers` | `deploy-key-startup-check.test.ts`: "arm running-tier-mismatch: credentialTier L1, a still-running lane's marker does not carry a matching tier"; "arm running-tier-mismatch: a marker that never recorded a credential_tier counts as a mismatch"; "arm running-tier-mismatch: a mismatched marker with a dead pid still refuses"; "arm running-tier-mismatch: the running-lane scan itself failing" |
...
| Reconcile checks local file existence, remote id listing, matching normalized `<type> <base64>` key material, and SSH preflight — all four green before permission repair runs, the LAST gate. | `init.ts::reconcileDeployKey`, `enforceDeployKeyPermissions` | `init.test.ts`: "a discovered anchor already on disk with the wrong permissions (dir 0777, key 0777, sidecar 0777) has ALL THREE repaired to 0700/0600/0600" (repair order); "local .pub content does NOT match" (content mismatch) |
| Any reconcile failure is WARN-only and touches no file — not even the stale sidecar — remote never touched; a TTY offers (a) a fresh per-machine key or (b) degrade, non-interactive always (b). | `init.ts::armAuthFailsStaleOrMismatch` | `init.test.ts`: "RECONCILE FAILS (recorded id no longer listed — rotated/stale)" (no file modified); "operator chooses (a) -> registers an ADDITIONAL per-machine key titled sapwood-worker-<hostname>" |
| Branch protection is checked once provisioning/reconcile succeeds (legacy endpoint, then rulesets on a 404); WARN only if both report unprotected, distinct from a "cannot verify" WARN on any other read failure, and from a ruleset alone confirming protection. | `init.ts::checkDefaultBranchProtectionAction` | `init.test.ts`: "L1 active + default branch UNPROTECTED" (unprotected); "CANNOT BE VERIFIED" (cannot verify); "legacy protection 404s, but a RULESET covers the branch -> treated as protected, not confirmed-unprotected" (positive ruleset) |
| When more than one local sidecar exists (a stale primary plus a fresher per-host replacement), the most recently WRITTEN one wins as the anchor. | `config/paths.ts::findDeployKeyAnchor` | `paths.test.ts`: "when more than one sidecar exists (a stale primary plus a fresher per-host replacement), the most recently WRITTEN one wins" |
...
gh repo deploy-key list -R <owner>/<repo> --json id,title -q '.[] | select(.title=="sapwood-worker") | .id' | tail -1 > .sapwood/keys/worker-deploy-key.id
