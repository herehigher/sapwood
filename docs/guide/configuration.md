...
| `maxPoolRemovalAttempts` | `5` | Maximum recorded `labels.roundPool` removal failures for one issue since its most recent selection into a round pool. The round-open reconciliation and round-close sweep share this count. When the count reaches the cap, the engine skips another removal attempt, escalates the issue to `needs-human`, and leaves the stale pool label in place; held issues are excluded from pool eligibility, so the label no longer affects dispatch. |
...
| `verificationPlanDrafter.promptFile` | unset | Override the gate⓪ **drafting** session's prompt — a briefed, issues-only session distinct from the reviewer, invoked when the reviewer requests a plan draft (see `verificationPlanReviewer.maxDraftCycles` below). Never implements the issue, never approves its own draft. The same relative-path pattern as every other `promptFile` key. Unset uses the engine's shipped `prompts/verification-plan-drafter.md`. Shares the reviewer's `model`/`effort`/`fallbackModel` — every role session's own settings apply to its own session, this one included. |
...
| `architect.maxConsecutiveDrops` | `2` | Number of trailing, already-applied `drop` verdicts allowed for the same issue body. When the prior count is at least this value, the next `drop` still removes `labels.roundPool` and records the verdict, then escalates to `needs-human` instead of posting another ordinary drop-reason comment. An issue-body edit changes the recorded hash and resets the trailing count. |
...
| `po.maxConcernPostAttempts` | `5` | Maximum recorded posting failures for one durable concern, identified by its round, issue, and reason. Body-read, comment-read, and comment-write failures all count. While the count is below the cap, the next sweep retries; once it is at or above the cap, the next sweep skips another post attempt and escalates the issue to `needs-human`. |
...
disable a phase, and its own default-off logging is not covered by the paragraph above.
