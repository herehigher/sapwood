...
| `emptySpin.consecutiveDegradedRoundsThreshold` | `3` | The **empty-spin breaker** — this many consecutive closed rounds in a row that each dispatched zero workers AND had at least one peripheral role session fully degrade parks dispatch (the same `"llm"` park episode a classified quota/429 failure would enter), independent of whether the failure text matches any recognized error signature. Any round that isn't both idle and fully degraded resets the count to `0`. |
...
| `maxPoolRemovalAttempts` | `5` | Bound on retrying a failed `labels.roundPool` removal for the same issue (align.ts's round-open reconcile and round.ts's round-close sweep both route through it) — reaching it escalates the issue to `needs-human` instead of retrying forever, and leaves the stale label in place since it stops mattering once the issue is held. |
...
| `architect.maxConsecutiveDrops` | `2` | During the architect's round-pool batch review, the max consecutive `drop` verdicts it will apply to the SAME issue with an unchanged body (no edit in between) before escalating to `needs-human` instead — bounds the unbounded same-reason re-drop churn an issue would otherwise accumulate by re-entering the pool unchanged round after round. |
...
| `po.maxConcernPostAttempts` | `5` | Max recorded post failures the durable-concern retry sweep (`dissent.ts`'s `reconcileDurableConcerns`) tolerates for the same concern before giving up and escalating it to `needs-human` instead of retrying forever — the deterministic case is a permanently unreadable issue (deleted, transferred, locked); a transient failure retries and clears well under the cap. |
...
| `caps.maxAuditCommentsPerCall` | `20` (max `100`) | `pr_audit_comments`: the caller-visible return cap, applied AFTER audit-marker filtering. An explicit `lastN` above this is **rejected**, same reject-not-truncate contract as `maxCommentsPerCall`/`maxReviewThreadsPerCall`. |
| `caps.maxAuditCommentScanWindow` | `100` (max `100`) | `pr_audit_comments`: the underlying top-level-comment scan window fed to the forge read (GraphQL `last:`) BEFORE audit-marker filtering. Kept wider than `maxAuditCommentsPerCall` on purpose so ordinary comment spam can't crowd out audit-marker evidence before it's even found. |
