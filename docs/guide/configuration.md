...
`EMERGENCY_STOP`, `KILL_SWITCH`, `PAUSE`, and `ESCALATION` also live directly under the runtime
root and disappear with it. So does `sapwood.lock`, the single-instance lock: one runtime root =
one board = at most one running engine — a second `sapwood run` against the same root refuses to
start, and a stale lock from a crashed engine is taken over automatically once its pid is dead
(see [troubleshooting](troubleshooting.md#single-instance-lock)). Losing the database resets the
...
**Cache** is `.sapwood/cache/`: rebuildable evidence and materialized inputs (a private review
clone/worktrees, generated per-round role-skills plugin bundles), tagged with the standard
`CACHEDIR.TAG` signature so cache-aware backup/sync tooling skips it. Safe to delete only when no
engine runs against this root — a running engine may be mid-write to it.

**Engine-exclusive.** The whole tree is engine-owned: operator scratch (retro notes, local
scripts, ad hoc files) belongs elsewhere in the repo, never under `.sapwood/`. The directory
self-declares this with its own `.gitignore` (`*`, written by the engine/`sapwood init` on
creation) — it is never committed and never needs a rule in the repo's own root `.gitignore`.
...
**Guard.** The guard's write-deny rule fences the whole `.sapwood/` root as one unit; see
[Security & trust model — Sentinel isolation boundary](../security.md#sentinel-isolation-boundary-honest-statement)
for the rule itself (not restated here).
...
| Key | Default | Meaning |
|---|---|---|
| `path` | unset (`.sapwood/logs/sapwood.log`) | Destination for the run narrative. Optional — unset resolves against cwd, the runtime root's own log location; a SET relative path resolves against the config file's own directory instead. |
...
| `credentialTier` | `L0` | The governing L0/L1 decision for every worker leg's credential scope — `L0` or `L1` only. `L0` (default) retains the ambient forge/git credential channels the operator's own environment provides — the spawn env overlays background-task and guard variables on top (`worker.ts`'s `dispatch`/`resume`), it does not narrow what's inherited — and never reads or probes a deploy key at all. `L1` requires a local (key file, id sidecar) anchor `sapwood init` provisions under this machine's own `.sapwood/keys/` — never a value in this file, since WHERE the key lives is a fact about one machine, not a governing decision. See [Security & trust model](../security/credential-tiers.md#worker-credential-tiers) for the full tier table, spawn-env mechanics, and refusal conditions. |
...
| `maxPoolRemovalAttempts` | `5` | Maximum recorded `labels.roundPool` removal failures for one issue since its most recent selection into a round pool. The count resets when a pool-selection event includes the issue; each `pool-reconcile-incomplete` event then increments it. When the count reaches this cap, the engine skips another removal attempt and leaves the stale pool label in place. It attempts to apply `needs-human`, then records `round-pool-removal-capped` with `labeled: 1` or `labeled: 0`; once that terminal event exists, no further removal or label attempts are made. Only a successful label write excludes the held issue from pool eligibility; if the write fails, the stale pool label can still affect dispatch. |

A round directive — human steering (why/what direction; execution stays the agents') is a
plain data file at the fixed path `.sapwood/DIRECTIVE.md`, **not a config key** (no
`round.directiveFile` — a config that sets it fails the standard unknown-key error). Drop it
before or during a round; at round open the engine reads it, substitutes it into both the
aligning (`po.md`) and architecting (`architect.md`) prompts as `{{round.directive}}`, then
archives it to `.sapwood/directives/round-N.md` so it never silently re-applies to a later
round. Consume-once is event-sourced (a durable `directive-applied` event, not the file's
presence, is the source of truth), so a crash mid-archive is safe to resume. Absent -> prompts
render an explicit "No round directive was provided for this round." placeholder, behavior
otherwise unchanged.
...
| Key | Default | Meaning |
|---|---|---|
| `file` | `docs/GOAL.md` | Path to the project's north-star goal file (not to be confused with sapwood's own `docs/PLAN.md`, a different file). The same relative-path resolution as `worker.promptFile`: a relative path resolves against **the config file's own directory**, not the CLI's cwd. `sapwood init` scaffolds a starter template here — Goal / Non-goals / Constraints / Architecture / Current milestone, each a short commented section — **iff the resolved path is missing**; it never overwrites an existing file (a second `init` run, or a crash-rerun, is a byte-for-byte no-op once the file exists). For the aligning phase's goal-decomposition pass specifically, a missing/unreadable file is an **explicit, fail-closed failure** — no `po-align` session is spawned, no issues are created that pass, and a durable `goal-file-unreadable` event + a `tick-error` are recorded. This never wedges the round or blocks anything else: the round-start triage pass (which never reads this file) and every other peripheral proceed unaffected, and the next round's own aligning phase retries the read fresh. (The architecting peripheral's own, independent read of this file is unchanged — its excerpt is now the file's `## Constraints` section **plus** its `## Architecture` chapter, Constraints first, each degrading to its own visible placeholder string, never a blank one, on the same failure.) |
...
| `docs` | `en` | Working language for documentation files/chapters a role edits (e.g. the architect's goal-file Constraints/Architecture-section proposals). |
...
| `verificationPlanDrafter.promptFile` | unset | Override the gate⓪ **drafting** session's prompt — a briefed, issues-only session distinct from the reviewer, invoked when the reviewer requests a plan draft (see `verificationPlanReviewer.maxDraftCycles` below). Never implements the issue, never approves its own draft. The same relative-path pattern as every other `promptFile` key. Unset uses the engine's shipped `prompts/verification-plan-drafter.md`. Has its own independently configurable `model`, `effort`, and `fallbackModel`; their defaults match the reviewer's defaults, but reviewer overrides do not propagate to the drafter. |
| `*.model` | `sonnet` | Model each of the six peripheral role sessions (`verificationPlanReviewer`, `verificationPlanDrafter`, `architect`, `po`, `harvest`, `retro`) runs as, independent of `worker.model`/`reviewer.agent.model`. |
| `*.effort` | `medium` | `low` \| `medium` \| `high`. Effort each peripheral role session runs at, independent of `worker.effort`/`reviewer.agent.effort`. |
...
| `harvest.promptFile` | unset | Override harvest's prompt (writes issue comments only — see the `roles` section intro above for its no-shell posture). The same relative-path pattern as every other `promptFile` key. Unset uses the engine's shipped `prompts/harvest.md`. |
...
| `architect.promptFile` | unset | Override the architect's prompt (round design/review between goal alignment and dispatch — never reviews PR code, never merges). The same relative-path pattern as every other `promptFile` key. Unset uses the engine's shipped `prompts/architect.md`. |
...
| `architect.maxConsecutiveDrops` | `2` | Caps consecutive same-body drops from the architect. While the prior count is below this value, the drop removes the issue from the round pool, records the verdict, and posts the ordinary drop comment. At or above the cap, the next drop still removes the pool label and records the verdict first, then attempts `needs-human` instead of posting that comment. The label write is best-effort and the terminal escalation records `labeled: 1` or `labeled: 0`; if labeling fails, the issue is not held and may re-enter the pool. |
...
| `po.maxConcernPostAttempts` | `5` | Caps failed attempts to post one PO concern. The first failed post records attempt 1. While the recorded count is below this value, the next sweep retries; once it is at or above the cap, the next sweep skips posting and attempts to apply `needs-human`. The label write is best-effort, and `concern-post-escalated` records `labeled: 1` or `labeled: 0`; once that terminal event exists, the concern leaves the retry queue even if the label could not be applied. |
...
**`skills.enabled`** is a separate toggle, unrelated to the phase switches above: it does not
disable a phase, and `false` emits no startup log.

| Key | Default | Meaning |
|---|---|---|
| `skills.enabled` | `false` | `true` renders (or reuses) an engine-owned skills plugin — verbatim reference material pulled from this repo's own security doc, plus resolved label semantics — and attaches it to every worker leg and every non-review peripheral role session; review sessions never receive it. Rendering happens eagerly at engine startup and fails startup closed if the source doc or its markers are missing or malformed, rather than failing lazily on a session's first spawn. `false` (the default) attaches nothing — every session's invocation is unaffected by this feature's existence. |
...
| `caps.maxAuditCommentsPerCall` | `20` (max `100`) | `pr_audit_comments`: caller-visible return cap, applied after audit-marker filtering. An explicit `lastN` greater than this value is rejected, while equality is allowed. The same value also caps the newest trusted top-level comments searched before and after an engine-agent audit post when discovering its marker and recording the delivery receipt. |
| `caps.maxAuditCommentScanWindow` | `100` (max `100`) | `pr_audit_comments`: number of newest trusted top-level PR comments retained after the forge pages the comment connection and provenance-filters it, but before audit-marker filtering. If more trusted comments exist, the response reports `complete: false`; reaching the forge's fixed 50-page safety ceiling fails the read closed. The wider default leaves room for trusted non-audit comments without making the returned audit stream unbounded. |
...
The grant is **not** paired with settings pinning — see `docs/security/egress.md`'s peripheral-egress section
...
- **`llm`** — the probe is a **minimal inference ping** (same `CLAUDE_BIN` resolution as a real
  ...
  **Honest cost:** ~$0.016 per ping measured (the CLI still sends ~7.4k scaffolding tokens plus
  ...
  dispatch is paused (`.sapwood/PAUSE` blocks the canary the ping exists to unlock) — the free
...
engine's runtime root (written by the engine, read-only informational output, never a control input
