...
| `credentialTier` | `L0` | The governing L0/L1 decision for every worker leg's credential scope — `L0` or `L1` only. `L0` (default) is the full-credentialed behavior — every worker leg inherits the operator's env verbatim — and NEVER reads or probes a deploy key at all. `L1` requires a local (key file, id sidecar) anchor `sapwood init` provisions under this machine's own `.sapwood/keys/` — never a value in this file: WHERE the key lives is a fact about one machine, not a governing decision, so it is discovered fresh at startup rather than written back here. With `L1` set, every worker leg — dispatch, resume, AND fix — runs with `GIT_SSH_COMMAND` pinned to the discovered key (composed onto the credential-free base for a fix leg, never replacing its severing) and no forge API credential reachable in its env at all (`Bash(gh *)` drops out of the leg's tool grant too). **No silent downgrade to L0**: if `L1` is set and `sapwood run` finds no reconciled local anchor at startup (missing, unreadable, removed from the repo, listed read-only, or listed with write access simply unconfirmed), OR finds a lane still running from before this restart whose own persisted credential tier doesn't match (never silently trusting one that predates `credential_tier` at all, an unparseable marker, or a running-lane state directory that can't be listed), it refuses to start — before any dispatch or board/label mutation — naming `sapwood init` as the fix, or, for a mismatched running lane, naming that lane/session/pid and exactly two remedies: wait for it to exit (then delete the stale `.running.json` once it's actually gone), or `kill <pid>` first if you cannot wait — never `sapwood estop`, which cannot reach a lane once the engine that owned it is already dead; a preflight that later fails mid-run (past that startup gate) fails that one leg outright rather than falling back to the wider tier. See [Security & trust model](../security/credential-tiers.md#worker-credential-tiers) for the full tier table, the reconcile state machine, and residuals. |
