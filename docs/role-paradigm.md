...
`conductor.ts`'s RESUME phase, CAPPED branch, bounded by `lanes.capSplitPerRound` and never
for an issue already carrying the cap-split origin marker), admits one oversized issue,
