...
For the seven peripheral roles below, sapwood's deterministic engine (`runRounds`,
...
| **architect** <!-- sapwood:write-inventory-role:architect --> | `contradictions:[{issue, severe}]` + `verdicts:[{issue, verdict, reason}]` + design note + per-issue explanations → `addIssueComment(anchor, designNote)`; per contradiction `addIssueComment` + `addLabel(blocked)` when `severe`; per pool-member verdict `removeRoundPoolLabel` (`drop`) or `addLabel(needsHuman)` (`needs-human`) + `addIssueComment` — `architect.ts:921-989` | `validateArchitectOutput` (`architect.ts:235`): `ArchitectMetadataSchema` (strict) + `parseArchitectBody` fail-closed sub-format parse (`architect.ts:190`; own-line `<<<CONTRADICTION #n>>>`/`<<<VERDICT #n>>>` markers, no embedded sub-delimiters) **+ set cross-check** — every flagged `contradictions` issue must be inside the engine-computed candidate set, and every `verdicts` issue must SEPARATELY be inside this round's pool membership, or the whole output is rejected before any write (`architect.ts:270-278`) | **Medium** — the `blocked` label gates dispatch of the flagged issue; a `drop` verdict removes round-pool membership (idempotent) and a `needs-human` verdict adds a human-only-removable hold label; the design-note comments are advisory. Validation matches: schema + structural body parse + candidate-set/pool-set bound on each label target; the prose itself carries no truth check. |
