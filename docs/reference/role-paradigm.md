...
Of the seven peripheral roles below, six have no engine-granted GitHub write
channel: sapwood's deterministic engine (`runRounds`, `engine/src/loop/round.ts`)
applies GitHub writes from their validated session output. Retro is the
exception on engine-granted paths: its session may push a proposal branch
itself (`Bash(git push*)`, `retro.ts::RETRO_ALLOWED_TOOLS`), after which the
engine verifies the branch and opens or updates the PR.
