...
Of the seven peripheral roles below, six have all their GitHub writes applied by
sapwood's deterministic engine (`runRounds`, `engine/src/loop/round.ts`) from validated
session output — each of those role sessions is a scoped, bounded subordinate whose
judgment reaches GitHub only through the engine (#110). Retro is the exception: its
session may push a proposal branch itself (`Bash(git push*)`,
`retro.ts::RETRO_ALLOWED_TOOLS`), after which the engine verifies the branch and opens
or updates the PR.

The **worker is the write-capable exception** and is
...
history of how it got here; provenance is a one-line `#nn` reference only. This doc
was commissioned by the 2026-07-12 system design review (its findings are now folded
into [`docs/PLAN.md`](../PLAN.md), which superseded the standalone review record). See
this doc's own "[Validation depth ∝ decision
weight](#validation-depth--decision-weight-the-structured-output-write-inventory)"
section, below, for the standing structured-output write inventory (that table tracks
*what each role's output drives*; the rest of this doc explains *the shape every role's
session/output/idempotency/escalation must have*).
...
Three tiers, from strongest to weakest containment (the 2026-07-12 system design
review's "Authority answers live in layers" principle). Every role's writes sit at
exactly one tier; prefer moving a future role's writes UP this ladder over adding a
pattern-level deny.

1. **Unreachable (for ENGINE-GRANTED writes)** — no ENGINE-GRANTED write tool channel exists, for
   six of this doc's seven peripheral roles: po, po-pool, architect, verification-plan-reviewer,
   verification-plan-drafter, harvest. This is a claim about the `--allowedTools`/
   `--disallowedTools` grant these six roles' sessions receive from the engine, not about
   ...
   names — see [`security.md`](../security.md#accepted-blind-spots)'s residual-unknown-MCP-servers
   ...
   (#110; see [`security.md`](../security/role-sessions.md#issues-only-role-sessions-read-only-worktree-confined-no-shell)). This is
   ...
   [`security.md`](../security/role-sessions.md#issues-only-role-sessions-read-only-worktree-confined-no-shell)'s own note on this
...
2. **Intercepted** — a tool channel exists, but a fail-closed hook blocks the
   ...
   (`CLAUDE.md` non-negotiables). Six of the seven roles below — po, po-pool, architect, verification-plan-reviewer,
   verification-plan-drafter, harvest — sit at tier 1 and do NOT rely on this tier as their primary boundary;
   for them it is only the backstop under tier 1's absence-of-capability. `retro`, the seventh, does
...
3. **Choke point** — the role's session emits untrusted structured output (a
   ...
   role's own choke-point module and this doc's own "Validation depth ∝ decision
   weight" table, below, for the full output-field → write → validation inventory.
...
to its round/phase.
All roles except po-pool use the same `<!-- sapwood:round:{roundId}:{phase} -->` marker
...
**#796.** Missing from this section (and from the write-inventory table below) since the
...
| **architect** <!-- sapwood:write-inventory-role:architect --> | `contradictions:[{issue, severe}]` + `verdicts:[{issue, verdict}]` + design note + per-issue BODY explanations/reasons → `addIssueComment(anchor, designNote)`; per contradiction `addIssueComment` + `addLabel(blocked)` when `severe`; per verdict, `removeRoundPoolLabel` (`drop`) or `addLabel(needsHuman)` (`needs-human`), then `addIssueComment` with the BODY reason — except a `drop` whose unchanged body hits `roles.architect.maxConsecutiveDrops` escalates via `escalateToNeedsHuman` instead of posting the ordinary reason comment (`architect.ts:921-1020`) | `validateArchitectOutput` (`architect.ts:235`): `ArchitectMetadataSchema` (strict, `verdicts` carries only `{issue, verdict}` — the `reason` is parsed from the corresponding BODY section, `architect.ts:590-616`) + `parseArchitectBody` fail-closed sub-format parse (`architect.ts:190`; own-line `<<<CONTRADICTION #n>>>`/`<<<VERDICT #n>>>` markers, no embedded sub-delimiters) **+ set cross-check** — every flagged `contradictions` issue must be inside the engine-computed candidate set, and every `verdicts` issue must SEPARATELY be inside this round's pool membership, or the whole output is rejected before any write (`architect.ts:270-278`) | **Medium** — the `blocked` label gates dispatch of the flagged issue; a `drop` verdict removes round-pool membership (idempotent) and a `needs-human` verdict adds a human-only-removable hold label; the design-note comments are advisory. Validation matches: schema + structural body parse + candidate-set/pool-set bound on each label target; the prose itself carries no truth check. |
...
The rows above follow the round's own phase sequence (`round.ts`'s `SEQUENCE`: `aligning` — which
...
The per-role contract behind each row (responsibility, write-scope tier, marker idempotency,
output schema, escalation path) is the "Per-role sections" above, in this same document —
`docs/PLAN.md` keeps only a one-paragraph pointer to this section, not a restatement of it.

## Cross-cutting notes

- **Structured output format.** Six of the seven roles (all but retro) share one
...
- **Prompts are static with respect to `proxy.enabled` (#559, ruling 2026-08-03).** A role prompt
  ...
  opt-out is in [`configuration.md`](../guide/configuration.md#proxy).
- **What this doc is not.** This doc's own "Validation depth ∝ decision weight" table,
  above, is the standing structured-output write inventory (output field → `IForge`
  write → validation → decision weight) and the safety baseline every future
  write-widening change updates; `docs/PLAN.md` keeps only a one-paragraph pointer to
  it, not a second copy. This doc does not re-describe `guard.ts`'s enforcement
  mechanics (tokenizing, exec-prefix stripping, opaque-construct blocking) — see
  [`security.md`](../security.md). It does not cover the worker role (code-producing, a
  different session class entirely) or the runtime round-phase state machine
  (`docs/reference/loop-walkthrough.md`'s phase table) — this doc is about role
  CONTRACTS, not round RUNTIME STATES.

## What a v1.0 user-defined role must supply (issue #134, forward-looking)

The governed extension points backlog item (issue #134) depends on this doc. A
user-defined role — inserted before/after
...
prerequisite contract, written against what the seven built-in roles already do.
