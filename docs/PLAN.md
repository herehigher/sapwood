...
| 11 | Host-delegated capability management | **Amended 2026-08-03 (DR #616, owner-confirmed):** sapwood adopts host-delegated capability management — it abandons in-engine tool-permission/capability management for producer (worker) legs. Producer legs officially **inherit the operator's host Claude Code environment** — settings sources, MCP servers, skills — as documented behavior, not an accident of unset flags. **No `capabilities.*` config surface will ever be built.** Scope is **producer legs only**: the reviewer/peripheral sealing floor (gate② seal, Decision #10; #285/#410/#236 rulings) is untouched and stays non-negotiable. Rationale (owner): permission control is where complexity bugs live — don't over-invest; config-surface complexity hurts UX; this is an LLM-core product and cutting tools cuts the model's hands; favor empowerment under the trusted-repos-first threat model (Decision #3). A live probe against the worker's exact dispatch argv confirmed the premise (inherited MCP tools, including write/exec-class servers, are callable and arrive deferred rather than in the init inventory) before this decision was accepted. The engine keeps a small governance core instead — enumerated in [`docs/security.md`](security.md). **Amended 2026-08-19 (DR #1009, owner-confirmed, re-adjudicating [#304](https://github.com/herehigher/sapwood/issues/304) (c)):** two new host EXECUTION-PROFILE keys — `host.permissionMode: dontAsk\|auto\|bypassPermissions` (default `auto`) and `bashSandbox: host-managed\|required` (default `host-managed`) — are added alongside this decision, and are explicitly **not** a re-opening of it: a profile key configures HOW a session's already-granted tools reach the host (execution reach, credential-free-of-tool-choice), never WHICH tools a producer leg is offered. **Floor = governance core** (the two profile keys, like the five mechanisms `docs/security.md`'s "What stays engine-owned" section already enumerates, are engine-configured; `required` is checked at CLI-init time only via `failIfUnavailable`, not positively attested per leg, and `bypassPermissions` is an ungated operator call that triggers one guidance WARN naming the outer-boundary recipe, never an engine refusal); **allowances = host** (the actual `allowedDomains`/`allowRead`/`allowWrite` lists stay in the operator's Claude settings; `excludedCommands` does too except that `required`+L1 adds exactly `["git push *","git fetch *","git pull *","git ls-remote *"]` as an engine floor; no domain/path allowance surface is added to sapwood YAML). Full mechanics, the seven-layer table, the deployment-tier ladder, and the guaranteed-vs-residual lists for both keys live in [`docs/security.md`'s "Execution profiles"](security.md#execution-profiles-host-permission-mode--bash-sandbox) section. #304's rejection of egress isolation is re-adjudicated to: "Bash-channel containment available as a hardening profile; egress is not 'solved'; inherited MCP and hooks stay outside the boundary; the egress tripwire stays as forensics." **Amended 2026-08-20 (owner-confirmed):** the `bashSandbox` axis is deferred pre-release — no such config key ships and the engine injects no sandbox settings; the probed floor survives as the operator recipe in [`docs/security.md`'s "Execution profiles"](security.md#execution-profiles-host-permission-mode--bash-sandbox) section; `host.permissionMode` stands exactly as specified. |
| 12 | Producer PR-opening ownership | **Amended 2026-08-04 (#351 final ruling, owner-confirmed; #605):** engine-open-PR is promoted from a rescue path to the **ordinary path** for worker lanes — a worker's job ends at pushing its branch; the engine performs the forge-API PR-open from deterministic code, at reclaim (`forge.ts::associateLanePr` — branch known, no open PR on it, branch confirmed pushed via `probePushedBranch`, session over → the engine calls `forge.openPR` with an engine-authored body carrying the `sapwood:pr-owner` marker). The worker prompt no longer instructs `gh pr create`; a worker that opens its own PR anyway (possible at L0, where the `gh` grant remains; structurally impossible at L1, #606 — see docs/security.md's worker credential tiers) has it adopted via the marker, never duplicated — unchanged association behavior from #377. Motivated by the producer≠merger consequence of #606's L1 credential tier: a worker holding only a git-transport deploy key, no API credential at all, structurally cannot open a PR, making engine-open the only channel rather than an option — this decision precedes and unblocks #606, which has since shipped. No engine git invocation targets a worker worktree on this path (#69 unchanged): `associateLanePr`'s forge calls are pure API writes. |
...
- **Build/runtime:** npm workspaces (`engine` now; `dashboard` slots in at v0.2),
  strict NodeNext TypeScript, **Node ≥ 24** (floor declared on both root and the
  `engine` workspace, published to npm as bare `sapwood`). Runtime deps: `yaml` + `zod` only.
...
- **Omitted:** application-specific categories out of scope for a generic safety core.
...
- **Fail-closed by design:** the hook denies
...
- **`worker.ts`** — the only module touching the Claude CLI: headless `claude -p` in a git
  ...
  **process-tree kill** via `kill(-pid)`, stream-json cost
...
- **Scope boundaries:** `drive_decision` only — the PR-gate ACTION→action map + parity testing
  move to **M3** with `merge-driver.ts`. Deferred follow-ups: **#31** (double-failure rollback/requeue hardening),
...
- **The fix loop (#245/#246/#247, M9, shipped 2026-07-18/19):** review findings get one
  ...
  **The reviewer prompt states its own enforced/judged boundary (#454, R6 §6a).**
  ...
  unbounded. Every enforced row is traceable to a live check; the judged half is stated as
  unverifiable by construction rather than implied to be checked.
...
`enabled: true` (default since #551) attaches a real handle to both live drivers' own
fix-loop worker leg — a real `TickDeps.fixLegResume` (`mintProxy` + `renderFixPrompt`)
— and, in `runRoundsEngine` only, a real `RoleRunner` `defaultProxy` to every
peripheral role session; `runTickEngine` constructs no peripheral runner and attaches
no `defaultProxy` at all — two distinct seams, not one shared mechanism, and not
symmetric across the two drivers. `enabled: false` (explicit opt-out) stays fully
inert — no proxy server is ever constructed, and no other `proxy.*` key changes
runtime behavior. #253 originally shipped a third, middle state (`shadow: true`,
mintable-but-unattached, for a scoped bring-up harness). The reason #551 deleted it is
not that the bring-up transition had completed — it is that the state was semantically
empty from the start: the proxy's own mint (`proxy/mint.ts`'s `createProxyMint`) never
read `enabled` or `shadow` at all, so a scoped harness could always mint directly
regardless of production-attachment state, and the three production attachment guards'
only reads of `shadow` were as `enabled && !shadow`, which collapses to `enabled` once
...
The committee's keystone finding remains: sapwood's guard is built for a *trusted* model
...
- **Structural producer≠merger (not just the argv guard):** the merge is always
  executed by the Conductor, never the worker, backed by branch protection + a merge
  identity distinct from the worker, so the
...
one — bidirectionally, so neither a missing row nor an orphaned one can land unnoticed (#796).
...
- **`/sapwood-init` + `sapwood init`** must be credible and idempotent:
  ...
  3. **Idempotent, recovery-safe provisioning:** detect existing board/fields/options
     before creating; tolerate partial failure; safely re-runnable — board creation is
     automated end-to-end, never a manual step left to the human.
...
- **M0.5 — Minimal onboarding:** ✅ **delivered (PR #24).** `sapwood init` (auth
  ...
  manual board-creation step end-to-end. Key behaviors
...
- **M2 — Engine core:** ✅ **delivered (PRs #30, #32, #34; dogfood #35→#36).**
  ...
  (#26, hard-default/soft-opt-in). Row-for-row parity tests covering the scheduling core's
...
- **v0.2 (post-v1) — the open milestone, two workstreams.**
  ...
  **record the run** as the launch artifact. Scope: event schema + `GET /api/loop/state`
  ...
  using TanStack Query for polling, plus a replay player and charts (chart/application
  components are *new* design). History-aggregation metrics
  (merge/rework/cycle-time) are a later phase, gated on GitHub-history extraction work
  that does not exist yet. Because workers may touch security-
...
**PO decomposition and issue granularity (#310).** Granularity is a *how* decision, so
splitting an oversized issue is engine-initiated, not human-initiated (owner ruling
2026-08-17): gate⓪'s `too_large` structured decision (#874, below) is the EARLY trigger —
judged post-Ready, before a lane is ever spent — and the resume-cap CAPPED branch (#965,
below) is the LATE one — judged after a lane exhausts its resume budget. A human may still
apply the prefix-aware `split` label directly as an override channel, but it is no longer the
main path either trigger routes around. One issues-only PO session returns a bounded set of
...
Further generations happen when `split` (engine- or human-applied) fires again on a remainder
or on a still-oversized child — depth is emergent, one generation at a time; v1 has no
automatic multi-generation recursion inside a single decompose session.

**gate⓪ `too_large` -> engine split, the early trigger (#874).** Both the reviewer and the
drafter are one adversarial pair over ONE writer entry: on a `too_large` verdict, the
verification-plan-reviewer session applies `split` through the SAME write path #965 (below)
uses — journaled, idempotent (an issue already carrying `split`, from either trigger or a
human override, gets no second write and no second event) — records one registry event, and
the issue leaves plan review for the round with zero draft cycles spent; the existing
decompose session picks it up next round. Unlike the resume-cap path, this trigger does NOT
consult the cap-split origin marker: that marker exists solely to stop a budget-driven
re-split chain, while a `too_large` verdict is structural evidence about size — a cap-split
...
unless the issue already carries the cap-split origin marker (a child of an earlier cap-split
— the ONE gate, deliberately with no separate rate limit: cap-splits are already bounded by
lane count per round, since a cap-split needs a lane to exhaust `maxResumes` in the first
...
- **Process-tree kill:** `worker.ts` must kill the whole `claude` subtree (process
  groups) — a plain PID kill leaves orphaned child processes running.
...
- **Guard:** `guard.ts` reproduces the full captured bypass-attempt suite; differential/fuzz vs
...
- **Engine parity:** the pure-function test suite (priority/blocker/selffeed/merge-decision)
  passes in TS.
...
- **End-to-end dogfood:** on a trusted throwaway repo — `init` (zero manual GitHub UI
  steps, from clean `gh auth`), seed 2–3 issues. Default run:
