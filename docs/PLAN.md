...
Reading the weight and validation columns TOGETHER is the whole point — not the row order, which
is round-pipeline sequence (decompose → gate⓪ → aligning/pool → architect → harvest → retro), not
a weight ranking (`architect`'s **Medium** row sits below `PO / aligning`'s **Low** one, for
example). The one **High**-weight output (verification-plan-reviewer's `plan:approved`) carries
the deepest validation of any row (schema + content invariant + identity); every other row's
validation depth is matched to THAT row's own weight, independent of where it sits in the table —
schema-only for a pure-comment write, schema + a set/bound cross-check for a label whose target
is engine-computed-bounded (architect's `blocked`, harvest's comment target, po-pool's round-pool
label), schema + a content invariant only where the write is itself the dispatch key. A future
change that adds a heavier write behind lighter validation — regardless of which row it lands in
