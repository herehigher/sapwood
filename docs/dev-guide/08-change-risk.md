...
| `config/config.ts` | `.strict()` schemas make changes user-visible; renamed/removed keys can break existing YAML. Update the commented starter `sapwood.config.example.yaml`, this repo's live `sapwood.config.yaml` if it pins the key, `docs/configuration.md`, validation, and defaults together. |
...
6. **Operator tunables live in config, not source constants.** `config/config.ts` defines named defaults and `sapwood.config.yaml` documents them; add corresponding material to `docs/configuration.md`.
...
- For new config keys, update `config.ts`, `sapwood.config.yaml`, validation tests, and `docs/configuration.md`.
