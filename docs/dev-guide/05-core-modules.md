...
`engine/src/roles/worker.ts` and `engine/src/roles/peripheral.ts` inject the compiled hook through inline Claude settings and refuse hard-mode startup if the artifact is missing. `guard.test.ts` is the bypass matrix; `guard.fuzz.test.ts` performs seeded differential fuzzing against a frozen, vendored snapshot of 0day's `guard.py` (`guard/fixtures/guard_py_snapshot/`, provenance in its `SOURCE.md`). The snapshot ships in-repo, so the differential tests need nothing but a local `python3` — no sibling `../0day` checkout. Point `SAPWOOD_ZERODAY_SRC` at a live checkout's `backend/src` to compare against a newer `guard.py`. This is a security boundary: read [Security](../security.md) before changing it, and treat it as human-merge-only.
