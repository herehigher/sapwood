...
sapwood is a Claude Code plugin bundle (slash commands, skills, the guard hook) around the `sapwood` engine CLI. Install the plugin (recommended), or run the CLI from npm on its own — both are complete paths.
