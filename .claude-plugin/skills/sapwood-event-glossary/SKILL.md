...
- `operator-fence-violated` — **intervene**: a role-proposed issue-body rewrite altered, removed, or reordered bytes inside an operator-owned `<!-- sapwood:operator-owned -->` fence; the write was refused (plan-review's reviewer/drafter paths escalate needs-human via `plan-review-escalated`, po-triage logs and skips the write). (see #827)
