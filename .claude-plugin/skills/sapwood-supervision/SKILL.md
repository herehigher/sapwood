...
Read [`docs/supervision.md`](https://github.com/herehigher/sapwood/blob/main/docs/supervision.md) before opening a batch, stopping the engine, or triaging queues — it is the canonical playbook for supervising a `sapwood run` session: the `status`/`events` poll-cursor recipe, batch open/close rituals, the stop ritual's sentinel placement/removal, `gh`-side queue queries, and the governance lines a supervisor operates under (including the trusted-operator ruling).
